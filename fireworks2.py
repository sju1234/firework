...
# 장치연화 개별 로켓에 쓰일 단발폭죽 후보 (클래식 개화 위주)
JANGCHI_INNER = [
    'peony', 'chrysanthemum', 'dahlia', 'ring', 'pistil',
    'taisho', 'bouquet', 'willow', 'kamuro',
]
...
SHAPES = [
    ...
    # ── 장치연화 (Formation Fireworks) ──────────────────────────────────────
    'jangchi_line',    # 장치연화 일렬   — 단발폭죽 N발을 직선으로 배치해 동시 발사
    'jangchi_fan',     # 장치연화 부채꼴 — 부채꼴(120°) 배열로 동시 발사
    'jangchi_v',       # 장치연화 V자   — V자형 좌우 대칭 배열 동시 발사
    'jangchi_circle',  # 장치연화 원형  — 원형 배열 동시 발사
    'jangchi_cross',   # 장치연화 십자  — 십자(+)형 배열 동시 발사
    # ── 특수 연출 효과 (Special Cinematic Effects) ────────────────────────────
    'aurora',        # Aurora         — 오로라 파동 커튼 (파란/초록/보라)
    'supernova',     # Supernova      — 초신성 대폭발 + 다중 충격파 링
    'helix',         # Helix          — 이중 나선 (DNA) 형태
    'vortex',        # Vortex         — 로그 나선 소용돌이 형태
    'galaxy',        # Galaxy         — 은하 나선팔 형태
    'snowflake',     # Snowflake      — 6각 대칭 눈꽃 결정
]
SHAPE_NAMES = [
    ...
    'Fountain',
    # 장치연화 (Formation)
    '장치 일렬', '장치 부채꼴', '장치 V자', '장치 원형', '장치 십자',
    # 특수 연출 효과 (Special Cinematic)
    'Aurora', 'Supernova', 'Helix', 'Vortex', 'Galaxy', 'Snowflake',
]
...
class Rocket:
    ...
    def explode(self):
        ...
        def p(vx,vy,vz, life, col=None, size=1.8, drag=0.978, grav=1.0, hot=0.28):
            ...
            for bi in range(n_balls):
                ...
                for _ in range(random.randint(3, 5)):
                    ...
                    p(vx2, vy2, vz2, random.randint(140, 195),
                      col=col2, drag=0.982, grav=0.2, size=20.0, hot=0.75)
            # 발화 순간 중심 화염 (고온 섬광)
            for _ in range(20):
                vx,vy,vz = sphere_v(random.uniform(0.2, 0.9))
                p(vx, vy, vz, random.randint(10, 28),
                  col=(1.0, random.uniform(0.65, 0.95), 0.15),
                  size=10.0, drag=0.990, grav=0.2, hot=0.96)

        # ── 팬 마인 (Mine Fan) ────────────────────────────────────────────────
        elif s == 'mine_fan':
            # 60° 넓은 부채꼴 상향, 팔레트 색상 별들
            n_balls = random.randint(60, 80)
            for _ in range(n_balls):
                phi2   = math.acos(random.uniform(0.85, 1.00))  # ±31° 이내
                theta2 = random.uniform(0, 2*math.pi)
                sp_ball = random.gauss(4.5, 0.80); sp_ball = max(2.0, sp_ball)
                bvx = sp_ball*math.sin(phi2)*math.cos(theta2)
                bvy = sp_ball*math.cos(phi2)
                bvz = sp_ball*math.sin(phi2)*math.sin(theta2)
                col2 = random.choice(pal)
                for _ in range(random.randint(2, 4)):
                    p(bvx + random.gauss(0, 0.10), bvy + random.gauss(0, 0.10),
                      bvz + random.gauss(0, 0.10), random.randint(120, 180),
                      col=col2, drag=0.980, grav=0.25, size=16.0, hot=0.65)
            for _ in range(25):
                vx,vy,vz = sphere_v(random.uniform(0.3, 1.1))
                p(vx, vy, vz, random.randint(8, 22),
                  col=(1.0, random.uniform(0.7, 1.0), 0.2),
                  size=9.0, drag=0.988, grav=0.3, hot=0.95)

        # ── 컬러 마인 (Mine Color) ────────────────────────────────────────────
        elif s == 'mine_color':
            # 팔레트 색상을 방위각 구역별로 나눠 쏘는 마인
            n_sectors = len(pal)
            balls_per_sec = random.randint(15, 22)
            for si, col2 in enumerate(pal):
                sector_start = (si / n_sectors) * 2 * math.pi
                sector_end   = ((si + 1) / n_sectors) * 2 * math.pi
                for _ in range(balls_per_sec):
                    phi2   = math.acos(random.uniform(0.90, 1.00))
                    theta2 = random.uniform(sector_start, sector_end)
                    sp_ball = random.gauss(4.0, 0.7); sp_ball = max(1.8, sp_ball)
                    bvx = sp_ball*math.sin(phi2)*math.cos(theta2)
                    bvy = sp_ball*math.cos(phi2)
                    bvz = sp_ball*math.sin(phi2)*math.sin(theta2)
                    for _ in range(random.randint(3, 5)):
                        p(bvx + random.gauss(0, 0.08), bvy + random.gauss(0, 0.08),
                          bvz + random.gauss(0, 0.08), random.randint(130, 200),
                          col=col2, drag=0.981, grav=0.22, size=18.0, hot=0.70)
            for _ in range(18):
                vx,vy,vz = sphere_v(random.uniform(0.2, 0.8))
                p(vx, vy, vz, random.randint(8, 20),
                  col=(1.0, 1.0, 0.9), size=8.0, drag=0.990, grav=0.2, hot=0.98)

        # ── 나선 마인 (Mine Spiral) ───────────────────────────────────────────
        elif s == 'mine_spiral':
            # 나선형 패턴으로 상승하는 마인
            n_turns = random.randint(2, 3)
            n_per_turn = random.randint(18, 26)
            total = n_turns * n_per_turn
            for i in range(total):
                t = i / total
                theta2 = t * n_turns * 2 * math.pi
                phi2 = math.acos(max(0.75, 1.0 - t * 0.30))
                sp_ball = random.gauss(4.2, 0.6); sp_ball = max(2.0, sp_ball)
                bvx = sp_ball*math.sin(phi2)*math.cos(theta2)
                bvy = sp_ball*math.cos(phi2)
                bvz = sp_ball*math.sin(phi2)*math.sin(theta2)
                col2 = pal[i % len(pal)]
                for _ in range(random.randint(2, 4)):
                    p(bvx + random.gauss(0, 0.07), bvy + random.gauss(0, 0.07),
                      bvz + random.gauss(0, 0.07), random.randint(120, 190),
                      col=col2, drag=0.982, grav=0.20, size=14.0, hot=0.60)
            for _ in range(20):
                vx,vy,vz = sphere_v(random.uniform(0.2, 0.9))
                p(vx, vy, vz, random.randint(8, 24),
                  col=(1.0, 0.95, 0.3), size=9.0, drag=0.989, grav=0.25, hot=0.96)

        # ── 크래클 마인 (Mine Crackle) ────────────────────────────────────────
        elif s == 'mine_crackle':
            # 지직거리는 은백색 크래클 마인
            SILVER = [(1.0,1.0,1.0),(0.90,0.90,1.0),(0.85,0.92,1.0),
                      (1.0,1.0,0.80),(0.80,0.85,1.0)]
            n_balls = random.randint(70, 100)
            for _ in range(n_balls):
                phi2   = math.acos(random.uniform(0.80, 1.00))  # ±37° 이내
                theta2 = random.uniform(0, 2*math.pi)
                sp_ball = random.gauss(5.0, 1.0); sp_ball = max(2.0, sp_ball)
                bvx = sp_ball*math.sin(phi2)*math.cos(theta2)
                bvy = sp_ball*math.cos(phi2)
                bvz = sp_ball*math.sin(phi2)*math.sin(theta2)
                col2 = random.choice(SILVER)
                p(bvx, bvy, bvz, random.randint(100, 160),
                  col=col2, drag=0.979, grav=0.35, size=12.0, hot=0.55)
                for _ in range(random.randint(2, 5)):
                    jx = bvx + random.gauss(0, 0.4)
                    jy = bvy + random.gauss(0, 0.4)
                    jz = bvz + random.gauss(0, 0.4)
                    parts.append(Particle(ox, oy, oz, jx, jy, jz,
                                          random.choice(SILVER),
                                          random.randint(20, 45),
                                          size=4.0, drag=0.965, grav=0.8,
                                          hot=0.35, spark=True, strobe=True))
            for _ in range(30):
                vx,vy,vz = sphere_v(random.uniform(0.3, 1.2))
                p(vx, vy, vz, random.randint(6, 18),
                  col=(1.0, 1.0, 1.0), size=7.0, drag=0.988, grav=0.3, hot=0.99)

        # ── 드래곤 마인 (Mine Dragon) ─────────────────────────────────────────
        elif s == 'mine_dragon':
            # 적금색 와이드 콘, 강한 중력으로 포물선 호를 그리는 마인
            DRAGON = [(1.0,0.20,0.05),(1.0,0.50,0.00),(1.0,0.80,0.10),
                      (0.90,0.10,0.00),(1.0,0.40,0.00),(0.80,0.05,0.00)]
            n_balls = random.randint(50, 70)
            for _ in range(n_balls):
                phi2   = math.acos(random.uniform(0.75, 1.00))  # ±41° 이내
                theta2 = random.uniform(0, 2*math.pi)
                sp_ball = random.gauss(5.5, 1.0); sp_ball = max(2.5, sp_ball)
                bvx = sp_ball*math.sin(phi2)*math.cos(theta2)
                bvy = sp_ball*math.cos(phi2)
                bvz = sp_ball*math.sin(phi2)*math.sin(theta2)
                col2 = random.choice(DRAGON)
                for _ in range(random.randint(4, 7)):
                    fade = random.uniform(0.6, 1.0)
                    p(bvx*fade + random.gauss(0, 0.12),
                      bvy*fade + random.gauss(0, 0.08),
                      bvz*fade + random.gauss(0, 0.12),
                      random.randint(100, 160), col=col2,
                      drag=0.974, grav=0.8, size=22.0, hot=0.80)
            for _ in range(30):
                vx,vy,vz = sphere_v(random.uniform(0.5, 1.5))
                p(vx, vy, vz, random.randint(10, 25),
                  col=(1.0, random.uniform(0.3, 0.8), 0.0),
                  size=12.0, drag=0.985, grav=0.5, hot=0.95)

        # ── 타이탄 마인 (Mine Titan) ──────────────────────────────────────────
        elif s == 'mine_titan':
            # 초대형 전방위 폭발 (위 반구 편중)
            TITAN = [(1.0,0.95,0.40),(1.0,0.80,0.10),(1.0,1.0,0.70),
                     (1.0,0.60,0.00),(0.90,0.75,0.20),(1.0,0.95,0.55)]
            n_balls = random.randint(90, 130)
            for _ in range(n_balls):
                phi2   = math.acos(random.uniform(-0.10, 1.00))  # 위 반구 + 약간 수평
                theta2 = random.uniform(0, 2*math.pi)
                sp_ball = random.gauss(6.0, 1.2); sp_ball = max(3.0, sp_ball)
                bvx = sp_ball*math.sin(phi2)*math.cos(theta2)
                bvy = sp_ball*math.cos(phi2)
                bvz = sp_ball*math.sin(phi2)*math.sin(theta2)
                col2 = random.choice(TITAN)
                for _ in range(random.randint(3, 6)):
                    p(bvx + random.gauss(0, 0.14), bvy + random.gauss(0, 0.10),
                      bvz + random.gauss(0, 0.14), random.randint(150, 230),
                      col=col2, drag=0.984, grav=0.18, size=24.0, hot=0.75)
            for _ in range(40):
                vx,vy,vz = sphere_v(random.uniform(0.4, 1.5))
                p(vx, vy, vz, random.randint(10, 30),
                  col=(1.0, random.uniform(0.8, 1.0), 0.4),
                  size=14.0, drag=0.990, grav=0.2, hot=0.98)

        # ── 비 마인 (Mine Rain) ───────────────────────────────────────────────
        elif s == 'mine_rain':
            # 빠르게 수직 상승 후 강한 중력으로 비처럼 쏟아지는 마인
            RAIN = [(0.50,0.70,1.00),(0.70,0.90,1.00),(1.00,1.00,1.00),
                    (0.40,0.60,1.00),(0.60,0.80,0.90)]
            n_balls = random.randint(50, 70)
            for _ in range(n_balls):
                phi2   = math.acos(random.uniform(0.90, 1.00))  # 좁은 수직 콘
                theta2 = random.uniform(0, 2*math.pi)
                sp_ball = random.gauss(7.0, 1.2); sp_ball = max(4.0, sp_ball)
                bvx = sp_ball*math.sin(phi2)*math.cos(theta2)
                bvy = sp_ball*math.cos(phi2)
                bvz = sp_ball*math.sin(phi2)*math.sin(theta2)
                col2 = random.choice(RAIN)
                for _ in range(random.randint(3, 5)):
                    p(bvx + random.gauss(0, 0.10), bvy + random.gauss(0, 0.10),
                      bvz + random.gauss(0, 0.10), random.randint(200, 280),
                      col=col2, drag=0.990, grav=2.8, size=10.0, hot=0.45)
            for _ in range(25):
                vx,vy,vz = sphere_v(random.uniform(0.3, 1.0))
                p(vx, vy, vz, random.randint(8, 20),
                  col=(0.80, 0.90, 1.00), size=7.0, drag=0.990, grav=0.3, hot=0.96)

        # ── 꽃 마인 (Mine Flower) ─────────────────────────────────────────────
        elif s == 'mine_flower':
            # 꽃잎 형태로 퍼지는 마인 (팔레트 색으로 각 꽃잎을 구분)
            n_petals = random.randint(5, 8)
            per_petal = random.randint(12, 18)
            for pi_idx in range(n_petals):
                petal_az   = (pi_idx / n_petals) * 2 * math.pi
                petal_elev = random.uniform(math.pi * 0.15, math.pi * 0.28)
                col2 = pal[pi_idx % len(pal)]
                for j in range(per_petal):
                    t = j / per_petal
                    narrow = 1.0 - abs(t - 0.5) * 2  # 꽃잎 중앙이 넓고 끝이 좁음
                    petal_phi = petal_elev * (0.5 + 0.5 * narrow)
                    sp_ball = random.gauss(4.0, 0.5) * (0.6 + 0.4 * narrow)
                    sp_ball = max(1.5, sp_ball)
                    theta2 = petal_az + random.gauss(0, 0.15)
                    bvx = sp_ball*math.sin(petal_phi)*math.cos(theta2)
                    bvy = sp_ball*math.cos(petal_phi)
                    bvz = sp_ball*math.sin(petal_phi)*math.sin(theta2)
                    for _ in range(random.randint(2, 4)):
                        p(bvx + random.gauss(0, 0.06), bvy + random.gauss(0, 0.06),
                          bvz + random.gauss(0, 0.06), random.randint(130, 200),
                          col=col2, drag=0.982, grav=0.22, size=15.0, hot=0.65)
            for _ in range(20):
                vx,vy,vz = sphere_v(random.uniform(0.2, 0.8))
                p(vx, vy, vz, random.randint(8, 22),
                  col=(1.0, 1.0, 0.80), size=8.0, drag=0.990, grav=0.2, hot=0.97)
        ...
        # ── 오로라 (Aurora) ───────────────────────────────────────────────────
            ...
            AURORA_C = [
                (0.15, 1.0, 0.55), (0.10, 0.85, 0.80), (0.30, 0.60, 1.0),
                (0.60, 0.15, 1.0), (0.15, 1.0, 0.75), (0.05, 0.90, 0.45),
            ]
            n_curtains = random.randint(8, 14)
            for ci in range(n_curtains):
                theta2 = (ci / n_curtains) * 2 * math.pi + random.gauss(0, 0.15)
                dist   = random.uniform(0.55, 1.30)
                col2   = AURORA_C[ci % len(AURORA_C)]
                perp   = theta2 + math.pi / 2
                n_v    = random.randint(40, 60)
                for k2 in range(n_v):
                    t    = k2 / n_v
                    wave = math.sin(t * math.pi * 3.5 + random.uniform(0, 0.8)) * 0.10
                    vx2  = (dist * math.cos(theta2) + wave * math.cos(perp)) * random.gauss(0.95, 0.12)
                    vz2  = (dist * math.sin(theta2) + wave * math.sin(perp)) * random.gauss(0.95, 0.12)
                    vy2  = (t - 0.5) * 2.8 + random.gauss(0, 0.10)
                    life = random.randint(130, 220)
                    parts.append(Particle(ox, oy, oz, vx2 * _bsz, vy2 * _bsz, vz2 * _bsz,
                                          col2, life, size=2.5,
                                          drag=0.993, grav=0.04 * _gmul, hot=0.04))
            for _ in range(30):
                vx2, vy2, vz2 = sphere_v(random.uniform(0.05, 0.28))
                c2 = random.choice(AURORA_C)
                parts.append(Particle(ox, oy, oz, vx2 * _bsz, vy2 * _bsz, vz2 * _bsz,
                                      c2, random.randint(30, 60), size=3.0,
                                      drag=0.988, grav=0.08 * _gmul, hot=0.15))

        # ── 초신성 (Supernova) ────────────────────────────────────────────────
        elif s == 'supernova':
            # 1단계: 거대 구형 폭발
            for _ in range(random.randint(700, 900)):
                sp = random.gauss(1.6, 0.30); sp = max(0.4, sp)
                vx2, vy2, vz2 = sphere_v(sp)
                c2 = random.choice(pal + [(1.0, 1.0, 1.0), (1.0, 0.9, 0.5)])
                p(vx2, vy2, vz2, random.randint(80, 130), col=c2,
                  drag=0.975, grav=0.8, size=2.4, hot=0.45)
            # 2단계: 다중 충격파 링 (3겹)
            for ring_r, ring_life, ring_sz in [(1.2, 22, 1.6), (2.0, 16, 1.2), (2.8, 11, 0.9)]:
                n_ring = random.randint(100, 140)
                for i in range(n_ring):
                    theta2 = (i / n_ring) * 2 * math.pi + random.gauss(0, 0.03)
                    sv = random.gauss(ring_r, 0.04)
                    p(sv * math.cos(theta2), random.gauss(0, 0.015), sv * math.sin(theta2),
                      ring_life, col=(1.0, 1.0, 1.0),
                      drag=0.968, grav=0.3, size=ring_sz, hot=0.95)
            # 3단계: 방사 빛살
            n_rays = random.randint(10, 16)
            for ri in range(n_rays):
                theta_r = (ri / n_rays) * 2 * math.pi
                streamer(theta_r, n=28, spread=0.06)
            # 중심 초대섬광
            for _ in range(50):
                vx2, vy2, vz2 = sphere_v(random.uniform(0.1, 0.5))
                parts.append(Particle(ox, oy, oz, vx2 * _bsz, vy2 * _bsz, vz2 * _bsz,
                                      (1.0, 1.0, 1.0), random.randint(5, 14),
                                      size=5.0, drag=0.980, grav=0.2, hot=1.0))

        # ── 헬릭스 (Helix) ────────────────────────────────────────────────────
        elif s == 'helix':
            # 두 나선이 반대 방향으로 감기는 이중 나선
            n_turns = random.randint(3, 5)
            n_per   = random.randint(20, 30)
            total   = n_turns * n_per
            r_helix = random.uniform(0.80, 1.20)
            h_helix = random.uniform(2.2, 3.4)
            col_a   = pal[0]
            col_b   = pal[-1]
            for i in range(total):
                t = i / total
                for sign, col2 in [(+1, col_a), (-1, col_b)]:
                    angle = sign * t * n_turns * 2 * math.pi
                    vx2 = r_helix * math.cos(angle) + random.gauss(0, 0.035)
                    vz2 = r_helix * math.sin(angle) + random.gauss(0, 0.035)
                    vy2 = (t - 0.5) * h_helix + random.gauss(0, 0.035)
                    p(vx2, vy2, vz2, random.randint(80, 125), col=col2,
                      drag=0.982, grav=0.55, size=2.2, hot=0.20)
            # 나선 중심축 별
            for k2 in range(24):
                t = k2 / 24
                vy2 = (t - 0.5) * h_helix
                p(random.gauss(0, 0.04), vy2, random.gauss(0, 0.04),
                  random.randint(55, 95), col=(1.0, 1.0, 0.9),
                  drag=0.986, grav=0.4, size=1.8, hot=0.30)

        # ── 보텍스 (Vortex) ───────────────────────────────────────────────────
        elif s == 'vortex':
            # 로그 나선 소용돌이: 안쪽에서 바깥으로 점점 넓어지며 회전
            n_arms = random.randint(3, 5)
            n_per_arm = random.randint(45, 65)
            b_log  = random.uniform(0.20, 0.32)
            for arm in range(n_arms):
                base_angle = (arm / n_arms) * 2 * math.pi
                col2 = pal[arm % len(pal)]
                for k2 in range(n_per_arm):
                    t      = k2 / n_per_arm
                    theta2 = base_angle + t * 3.5 * math.pi
                    r      = min(0.08 * math.exp(b_log * theta2), 2.0)
                    vx2    = r * math.cos(theta2) + random.gauss(0, 0.04)
                    vz2    = r * math.sin(theta2) + random.gauss(0, 0.04)
                    vy2    = random.gauss(0.0, 0.06)
                    life   = max(20, int(random.randint(55, 110) * (1.0 - t * 0.25)))
                    p(vx2, vy2, vz2, life, col=col2,
                      drag=0.979, grav=0.65, size=2.0 + t * 0.8, hot=0.22)
            for _ in range(50):
                vx2, vy2, vz2 = sphere_v(random.uniform(0.04, 0.22))
                p(vx2, vy2, vz2, random.randint(25, 55), size=3.0, drag=0.985)

        # ── 갤럭시 (Galaxy) ───────────────────────────────────────────────────
        elif s == 'galaxy':
            # 은하 나선팔: 2~3개의 로그 나선팔 + 밝은 핵
            GALAXY_C = [(1.0, 0.95, 0.75), (1.0, 0.85, 0.55), (1.0, 1.0, 1.0),
                        (0.80, 0.88, 1.0), (0.75, 0.60, 1.0)]
            n_arms  = random.randint(2, 3)
            n_stars = random.randint(320, 440)
            b_gal   = 0.25
            for star in range(n_stars):
                arm    = star % n_arms
                t      = random.uniform(0, 1)
                theta2 = (arm / n_arms) * 2 * math.pi + t * 2.8 * math.pi + random.gauss(0, 0.28)
                r      = min(0.05 * math.exp(b_gal * theta2), 2.0)
                vx2    = r * math.cos(theta2) + random.gauss(0, 0.04)
                vz2    = r * math.sin(theta2) + random.gauss(0, 0.04)
                vy2    = random.gauss(0.0, 0.04)
                sz     = max(1.2, 2.5 - r * 0.5)
                c2     = random.choice(GALAXY_C)
                p(vx2, vy2, vz2, random.randint(70, 120), col=c2,
                  drag=0.983, grav=0.45, size=sz, hot=0.15 + max(0, (1.0 - r * 0.5)) * 0.15)
            # 핵 — 밝은 중심 성단
            for _ in range(65):
                vx2, vy2, vz2 = sphere_v(random.uniform(0.02, 0.18))
                p(vx2, vy2, vz2, random.randint(55, 95),
                  col=(1.0, 1.0, 0.95), drag=0.986, grav=0.35, size=3.0, hot=0.50)

        # ── 눈꽃 (Snowflake) ──────────────────────────────────────────────────
        elif s == 'snowflake':
            SNOW_C  = [(1.0, 1.0, 1.0), (0.85, 0.92, 1.0), (0.90, 0.96, 1.0), (0.78, 0.88, 1.0)]
            n_main  = 6
            arm_len = random.uniform(0.90, 1.30)
            n_per   = random.randint(18, 26)
            n_branch = random.randint(2, 3)
            for arm in range(n_main):
                theta2 = (arm / n_main) * 2 * math.pi
                # 주 팔
                for k2 in range(n_per):
                    t   = k2 / n_per
                    vx2 = arm_len * t * math.cos(theta2) + random.gauss(0, 0.018)
                    vz2 = arm_len * t * math.sin(theta2) + random.gauss(0, 0.018)
                    vy2 = random.gauss(0.0, 0.025)
                    p(vx2, vy2, vz2, random.randint(75, 115), col=random.choice(SNOW_C),
                      drag=0.983, grav=0.50, size=2.1, hot=0.18)
                # 곁가지 (60° 방향으로 좌우 대칭)
                for bi in range(1, n_branch + 1):
                    bt  = bi / (n_branch + 1)
                    bx0 = arm_len * bt * math.cos(theta2)
                    bz0 = arm_len * bt * math.sin(theta2)
                    bl  = arm_len * (0.30 - bi * 0.04)
                    for sign in [+1, -1]:
                        bth = theta2 + sign * math.pi / 3
                        nb  = random.randint(8, 14)
                        for k3 in range(nb):
                            tb  = k3 / nb
                            vx2 = bx0 + bl * tb * math.cos(bth) + random.gauss(0, 0.014)
                            vz2 = bz0 + bl * tb * math.sin(bth) + random.gauss(0, 0.014)
                            vy2 = random.gauss(0.0, 0.020)
                            p(vx2, vy2, vz2, random.randint(65, 105), col=random.choice(SNOW_C),
                              drag=0.983, grav=0.50, size=1.7, hot=0.14)
            # 중심 섬광
            for _ in range(30):
                vx2, vy2, vz2 = sphere_v(random.uniform(0.03, 0.18))
                p(vx2, vy2, vz2, random.randint(30, 60),
                  col=(1.0, 1.0, 1.0), drag=0.986, grav=0.45, size=2.8, hot=0.55)

        # mine 계열 / comet / strobe / star / fish: 자체 효과로 완결 → 공통 섬광 전부 생략
        if s.startswith('mine') \
                or s in ('comet', 'strobe', 'star', 'fish', 'falling_leaves',
                         'aurora', 'helix', 'galaxy', 'snowflake'):
            return parts
...
class PlacedFirework:
    ...
    def make_rockets(self):
        ...
        # ── 장치연화: 여러 발사 위치에서 단발폭죽을 배치·동시 발사 ──────────────
        if shape_name.startswith('jangchi_'):
            pal      = self.effective_palette()
            n        = max(3, self.arm_count) if self.arm_count >= 2 else 5
            spacing  = 6.0                        # 발사대 간격 (월드 유닛)
            sub_shp  = random.choice(JANGCHI_INNER)

            # 패턴별 (dx, dz) 오프셋 목록
            offsets = []
            if shape_name == 'jangchi_line':
                # 일렬: x축 방향 직선
                for i in range(n):
                    offsets.append((spacing * (i - (n - 1) / 2.0), 0.0))

            elif shape_name == 'jangchi_fan':
                # 부채꼴: 120° 부채꼴로 퍼짐
                spread = math.radians(120)
                radius = spacing * (n - 1) / 2.0
                for i in range(n):
                    ang = -spread / 2 + spread * i / max(1, n - 1)
                    offsets.append((radius * math.sin(ang),
                                    radius * (1.0 - math.cos(ang)) * 0.3))

            elif shape_name == 'jangchi_v':
                # V자: 중심에서 좌우로 뻗는 두 팔
                half = (n + 1) // 2
                for i in range(half):
                    offsets.append(( spacing * i * 0.9,  spacing * i * 0.55))
                for i in range(1, n - half + 1):
                    offsets.append((-spacing * i * 0.9,  spacing * i * 0.55))

            elif shape_name == 'jangchi_circle':
                # 원형: 균등 분할
                r_c = spacing * n / (2 * math.pi)
                for i in range(n):
                    ang = (i / n) * 2 * math.pi
                    offsets.append((r_c * math.cos(ang), r_c * math.sin(ang)))

            elif shape_name == 'jangchi_cross':
                # 십자: 가로 + 세로 (중심 한 번만)
                arm_n = max(2, n // 4)
                for i in range(-arm_n, arm_n + 1):
                    offsets.append((spacing * i, 0.0))
                for i in range(-arm_n, arm_n + 1):
                    if i != 0:
                        offsets.append((0.0, spacing * i))

            rockets = []
            for dx, dz in offsets:
                target_y = random.uniform(h * 0.88, h * 1.12)
                r = Rocket(self.x + dx, self.z + dz, target_y, sub_shp, pal)
...
class Game:
    ...
    def _draw_particles(self):
        ...
        # ─────────────────────────────────────────────────────────────────────
        # 패스 A: 글로우 헤일로 (ps*2.2) — 화학 발광색 + 따뜻한 고온 틴트
        # 패스 B: 코어 (ps) — 순수 화학색 + 화이트핫 블렌드
        # 두 패스만 사용해 성능/품질 균형 유지
        # ─────────────────────────────────────────────────────────────────────

        # 미리 계산: (alpha, hot_blend, r,g,b, x,y,z, spark) 튜플 목록
        # — 루프 1회로 두 패스에 재사용
        cache = []
        for p in self.particles:
            a = p.life / p.max_life
            if a < 0.15:
                a = (a / 0.15) ** 1.6
            # strobe 깜빡임: sin 파로 on/off 반복 (~5회/초 @ 60fps)
            if p.strobe:
            ...
            if p.hot > 0.0 and a_raw > (1.0 - p.hot):
                ...
                h = 0.0
            cache.append((a, h, p.r, p.g, p.b, p.x, p.y, p.z, p.spark))

        # ── 패스 A: 글로우 헤일로 ────────────────────────────────────────────
        glPointSize(max(2.0, ps * 2.2))
        glBegin(GL_POINTS)
        for (a, h, r, g, b, x, y, z, spark) in cache:
            if spark:
                glColor4f(1.0, 0.7, 0.1, a * 0.22)
            else:
                # 고온 구간: 따뜻한 황백으로 글로우, 냉각 후: 화학색 글로우
                gr = min(1.0, r + (1.0 - r) * h * 0.80)
                gg = min(1.0, g + (1.0 - g) * h * 0.65)
                gb = min(1.0, b * 0.6 + h * 0.15)
                glColor4f(gr, gg, gb, a * 0.18)
            glVertex3f(x, y, z)
        glEnd()

        # ── 패스 B: 코어 ─────────────────────────────────────────────────────
        glPointSize(max(1.0, ps))
        glBegin(GL_POINTS)
        for (a, h, r, g, b, x, y, z, spark) in cache:
            if spark:
                glColor4f(1.0, 0.75 + h * 0.25, 0.20 * a, a * 0.95)
            else:
                # 화이트핫 → 화학색 전환
                cr = min(1.0, r + (1.0 - r) * h)
                cg = min(1.0, g + (1.0 - g) * h * 0.88)
                cb = min(1.0, b + (1.0 - b) * h * 0.60)
                glColor4f(cr, cg, cb, a * 0.92)
            glVertex3f(x, y, z)
        glEnd()

        # ── 패스 C: 고온 화이트 핫 (young & hot 파티클만) ─────────────────
        glPointSize(max(0.8, ps * 0.42))
        glBegin(GL_POINTS)
        for (a, h, r, g, b, x, y, z, spark) in cache:
            if h > 0.20:
                glColor4f(1.0, 0.97, 0.88, h * a * 0.88)
                glVertex3f(x, y, z)
        glEnd()
    ...
    def _draw_daw(self, surf):
        ...
        def color_swatch(i, sx2, sy2, sw2, active):
            r2,g2,b2 = (int(c*255) for c in PALETTES[i][0])
            pygame.draw.rect(surf,(r2,g2,b2),(sx2,sy2,sw2,sw2), border_radius=4)
            if active:
                pygame.draw.rect(surf, C_BLUE,(sx2-2,sy2-2,sw2+4,sw2+4), 2, border_radius=5)
        ...
        if self.mode == MODE_FW:
            ...
            surf.blit(self.font_xs.render("SHAPE", True, C_DIM),(8, pk_y+6))
            for i in range(len(SHAPE_NAMES)):
                ci,ri = i%COLS3, i//COLS3
                shape_btn(i, 8+ci*(BW3+4), pk_y+20+ri*(BH+4), BW3, BH, i==self.cur_shape)
            # COLOR — 실제 행 수 기반으로 gy 계산
            n_rows3 = (len(SHAPE_NAMES) + COLS3 - 1) // COLS3
            gy = pk_y+20+n_rows3*(BH+4)+8
            surf.blit(self.font_xs.render("COLOR", True, C_DIM),(8, gy))
            sw=26
            for i in range(len(PALETTES)):
                color_swatch(i, 8+i*(sw+4), gy+14, sw, i==self.cur_pal)
            # DELAY
            dy2 = gy+14+sw+10
            stepper("DELAY", f"{self.cur_delay:.1f}s", 8, dy2, PW, 'delay_dec','delay_inc')
